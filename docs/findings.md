...
**Severity:** Medium  
**Risk:** Timeouts exist, but there is no unified retry policy, exponential backoff+jitter, or circuit breaker around upstream LLM/API failures.  
**Exploit example:** Repeated upstream 429/5xx causes cascading user-visible failures without graceful degradation strategy.  
**Remediation:** Add idempotent retries for transient failures, circuit breaker per provider/tool class, and clear degraded-mode responses.
...
Gaps:
- No centralized retry/backoff/circuit breaker strategy.
